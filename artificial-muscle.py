...
class Run:
    ...
    @classmethod
    def plot_channel_widths(
        cls, before_bake: str, after_bake: str, suffix: str
    ) -> None:
        ...
        Figure.plot_conductivity(
            excel_filename=filename,
            sheet_name=after_bake,
            ax_obj=ax_obj,
        ...
        Figure.save(
            ...
            suffix=Widths.specs[suffix]["suffix"],
        )

    @classmethod
    def plot_widths_difference(
        cls, excel_filename: str, data_frame: pd.DataFrame, specs: dict
    ):
        """Create stem plots of differences recorded under two conditions."""
        experiment_specs = ChannelWidth(
            dataframe=data_frame, prior=specs["before"], past=specs["after"]
        )
        experiment_specs.get_differences()
        Figure.plot_differences(
            x=experiment_specs.differences["channel_id"].astype(
                "string"
            ),  # casting to string makes x-axis categorical
    ...
    @classmethod
    def plot_widths_fractional_difference2(
        cls, excel_filename: str, data_frame: pd.DataFrame, specs: dict
    ):
        ...
        Figure.plot_widths_fractional_differences(
            x=channel_width.differences["channel_id"].astype(
                "string"
            ),  # casting to string makes x-axis categorical
        ...
        Figure.save(
            excel_filename=excel_filename,
            dest=Config.output_dir,
            suffix=f"{specs['suffix']}_bar",
        )

    @classmethod
    def plot_channel_width_and_difference(
        cls, excel_filename: str, data_frame_: pd.DataFrame, specs_: dict
    ):
        """Plot channel widths and difference on same graph"""
        channel_width = ChannelWidth(
            dataframe=data_frame_, prior=specs_["before"], past=specs_["after"]
        )
        ...
        Figure.save(
            excel_filename=excel_filename,
            dest=Config.output_dir,
            suffix=specs_["suffix"],
        )

    @classmethod
    def plot_width_relative_error(
        cls, excel_filename: str, data_frame_: pd.DataFrame, specs_: dict
    ) -> None:
...
def main():
    ...
    parser.add_argument(
        "type", type=Type, choices=list(Type), help="Type of data to plot"
    )
    ...
    if args.type == Type.conductivity:
        ...
        is_file_save = (
            False  # with this switch figures are saved with each iteration of for loop
        )
        df = Figure.get_dataframe(excel_filename=filename)
        for key in Widths.specs:
            try:
                Run.plot_widths_fractional_difference2(
                    excel_filename=filename, data_frame=df, specs=Widths.specs[key]
                )
            except ValueError:
                logging.error(
                    "Config.py_all is either empty or has whitespace - set to existing sheet name"
                )
                sys.exit()
        ...
        Run.plot_channel_widths(
            before_bake="py_hp_sm_prior", after_bake="py_hp_sm_past", suffix="perp_sm"
        )
        is_file_save = False
        ...
        config_channel_study2()
        Run.plot_channel_widths(
            before_bake="py_vp_sm_prior", after_bake="py_vp_sm_past", suffix="para_sm"
        )
        is_file_save = False
        ...
        config_channel_study2()
        Run.plot_channel_widths(
            before_bake="py_hp_br_prior", after_bake="py_hp_br_past", suffix="perp_br"
        )
        is_file_save = False
        ...
        config_channel_study2()
        Run.plot_channel_widths(
            before_bake="py_vp_br_prior", after_bake="py_vp_br_past", suffix="para_br"
        )
        is_file_save = False
        ...
        for key in Widths.specs:
            try:
                Run.plot_channel_width_and_difference(
                    excel_filename=filename, data_frame_=df, specs_=Widths.specs[key]
                )
            except ValueError:
                logging.error(
                    "Config.py_all is either empty or has whitespace - set to existing sheet name"
    ...
    if is_file_save:
        Figure.save(
            excel_filename=args.filename, dest=Config.output_dir, suffix=str(args.type)
        )
